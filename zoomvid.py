...
# pickle is only needed for debugging
import pickle
import platform
import re
import sys
...
class Zoom:
    ...
    def get_meetings_by_user(self, userid, fromdate, todate):
        ...
        for start, end in per_delta(
            ...
            self.get_meetings_by_user_internal(userid, start.strftime("%Y-%m-%d"), end.strftime("%Y-%m-%d"))
    ...
    def format_filename(self, params):
        ...
        meeting_time_utc = dp.parse(params["start_time"]).replace(tzinfo=datetime.timezone.utc)
        meeting_time_local = meeting_time_utc.astimezone(ZoneInfo(self.zc.value("Recordings","timezone")))
        year = meeting_time_local.strftime("%Y")
        month = meeting_time_local.strftime("%m")
        day = meeting_time_local.strftime("%d")
        meeting_time = meeting_time_local.strftime(self.zc.value("Recordings","strftime"))
...
class DateDirectorySelector:
    def __init__(self, root, zoom):
        ...
        ttk.Label(root, text="Start Date:").grid(row=0, column=0, padx=10, pady=10, sticky="w")
        self.start_date = ttk.Entry(root, width=15)
        self.start_date.grid(row=0, column=1, padx=10, pady=10)

        # End Date Selection
        ttk.Label(root, text="End Date:").grid(row=1, column=0, padx=10, pady=10, sticky="w")
        ...
        # Directory Selection
        ttk.Label(root, text="Directory:").grid(row=2, column=0, padx=10, pady=10, sticky="w")
        self.directory = ttk.Entry(root, width=30)
        self.directory.grid(row=2, column=1, padx=10, pady=10)
        ttk.Button(root, text="Browse", command=self.browse_directory).grid(row=2, column=2, padx=10, pady=10)
    ...
    def show_progress_window(self, start_date, end_date, directory):
        ...
        ttk.Label(progress_window, text="Processing Files:").grid(row=0, column=0, padx=10, pady=10, sticky="w")
        ...
        ttk.Label(progress_window, text="File Name:").grid(row=1, column=0, padx=10, pady=10, sticky="w")
        ...
        ttk.Label(progress_window, text="Current File Size:").grid(row=2, column=0, padx=10, pady=10, sticky="w")
        ...
        ttk.Label(progress_window, text="File Progress:").grid(row=3, column=0, padx=10, pady=10, sticky="w")
        self.file_progress = ttk.Progressbar(progress_window, length=300, mode="determinate")
        self.file_progress.grid(row=3, column=1, padx=10, pady=10)

        ttk.Label(progress_window, text="Overall Progress:").grid(row=4, column=0, padx=10, pady=10, sticky="w")
        self.overall_progress = ttk.Progressbar(progress_window, length=300, mode="determinate")
        self.overall_progress.grid(row=4, column=1, padx=10, pady=10)

        ttk.Label(progress_window, text="Total Size:").grid(row=5, column=0, padx=10, pady=10, sticky="w")
        ...
        self.progress_ok_button = ttk.Button(progress_window, text="OK", state="disabled", command=progress_window.destroy)
        self.progress_ok_button.grid(row=6, column=1, pady=20, sticky="e")

        self.progress_cancel_button = ttk.Button(progress_window, text="Cancel", command=progress_window.destroy)
        ...
        threading.Thread(target=self.process, args=(start_date, end_date, directory, progress_window,)).start()
...
# Task complete
    ...
    def update(self, bytes_so_far, total_file_size):
        ...
        # Update overall progress
        self.overall_progress["value"] = (self.total_bytes_so_far + bytes_so_far) / self.total_size * 100
        #print(f"Update {bytes_so_far}, {total_file_size}, {self.total_bytes_so_far}, {self.total_size}")
    ...
    def process(self, start_date, end_date, directory, progress_window):
        ...
        for file in files:
            ...
            self.zoom.download_recording(file['download_url'],
                file['file_name'],
                file['file_size'],
                self.update
            )
...
def main():
    ...
    zoom = Zoom()
    root = tk.Tk()
    app = DateDirectorySelector(root, zoom)
    root.mainloop()
